...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 2 + abs((score-score%10)//10 - score%10)
...
def is_swap(score0, score1):
    ...
    if score0>1 and score1 >1:
        if not max(score0,score1) % min(score0, score1):
            return True
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if not player:
            ...
            score0 = score0 + take_turn(num0,score1,dice=dice)
        else:
            num1 =strategy1(score1, score0)
            score1 = score1 + take_turn(num1, score0,dice=dice)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if score > previous_score:
            incr = score - previous_score
            if incr > previous_high:
                if incr == 1:
                    print("1 point! That's the biggest gain yet for Player",who)
                else:
                    print(incr,"points! That's the biggest gain yet for Player",who)
                return announce_highest(who, previous_high=incr, previous_score=score)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    score=0
    max_avg = make_averaged(roll_dice, num_samples)
    for i in range(1,11):
        temp_score = max_avg(i, dice)
        if temp_score > score:
            score = temp_score
            num_dice = i
    return num_dice
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if free_bacon(opponent_score)>=margin and  \
                                not is_swap(free_bacon(opponent_score),opponent_score):
        return 0
