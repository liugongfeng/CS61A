...
def merge(lst1, lst2):
    ...
    def mergeSort(arr):
        ...
        while lft and rgt:
            if lft[-1]>rgt[-1]:
                res.append(lft.pop(-1))
            else:
                res.append(rgt.pop(-1))
        res.reverse()
...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [ create_row(columns) for e in range(rows) ]
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for i in range(max_rows-1, 0, -1):
        ...
        str_row = ''.join([player for i in range(num_connect)])
        return str_row in str_board
